...
from tests.manifests import extract_metadata_from_path  # noqa: E402
from tests.manifests import get_source_of_truth_filepath  # noqa: E402
...
def _load_pylock_packages(pylock_text: str, python_minor: str) -> dict[str, dict[str, Any]]:
    ...
    for p in doc.get("packages", []):
        if "marker" in p and not packaging.markers.Marker(p["marker"]).evaluate(marker_env):
            continue
        name = p["name"]
        assert name not in packages, f"duplicate {name}"
        packages[name] = p
    return packages
...
def _sha_for_tag(
    ...
    if suffix == "-n":
        return latest.get(ck)
    return released.get(ck)
...
def run_variant(variant: str, dry_run: bool) -> int:
    ...
    for wb in workbenches:
        ...
        for idx, (base_key, suffix) in enumerate(wb.versions):
            ...
            if shown is None:
                print(
                    f"skip {path.name} tag {idx}: no lockfile (tried worktree/git "
                    f"{'; '.join(rel_paths)})",
                    file=sys.stderr,
                )
                continue
            ...
            try:
                ...
                print(f"skip {path.name} tag {idx}: lockfile parse error: {e}", file=sys.stderr)
                continue
