...
class TestJupyterLabImage:
    ...
    def test_trash_cleanup_installed(self, jupyterlab_image: conftest.Image) -> None:
        container = WorkbenchContainer(image=jupyterlab_image.name, user=4321, group_add=[0])
        extension_check_pattern = r"^\s*odh_jupyter_trash_cleanup\b[^\n]*enabled\b"
        try:
            container.start(wait_for_readiness=False)
            exit_code, output = container.exec(["jupyter", "server", "extension", "list"])
            result_output = output.decode(errors="replace")
            assert exit_code == 0, f"`jupyter server extension list` failed:\n{result_output}"
