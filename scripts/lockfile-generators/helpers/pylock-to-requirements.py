...
Arguments:
    ...
    --sdist-hashes    el9-fallback (default): omit sdist hashes when an EL9-compatible
                      ...
                      prefer: always include sdist hashes so Hermeto source-only
                      prefetch can match artifacts (public-index baseline images).
...
def collect_index_hashes(pkg: dict, *, sdist_hashes: str = SDIST_HASHES_EL9_FALLBACK) -> list[str]:
    ...
    ``prefer``: always include sdist hashes when present (Hermeto source-only).
    """
    hashes: list[str] = []
    has_el9_wheel = False
